...
#include "LVGL/GUI_APP/lv_radar.h"
...
lv_obj_t *lv_radar_cont;
lv_timer_t *lv_radar_timer[6];
...
/**
  ...
  * @retval 无
  */
void lv_radar_back_btn_event_handler(lv_event_t *event)
{
    ...
    {
        lv_obj_del(lv_radar_cont);
        for (int i = 0 ;i < 6; i++)
        {
            lv_timer_del(lv_radar_timer[i]);
        }
...
void lv_interface_show(void)
{
    ...
    lv_radar_cont = lv_obj_create(lv_scr_act());
    lv_general_dev.lv_back_event = lv_radar_back_btn_event_handler;
    lv_general_dev.parent = lv_radar_cont;
    lv_general_dev.lv_general_win_create = lv_general_win_create;
    lv_general_dev.lv_general_win_create();
    /* 设置容器大小 */
    lv_obj_set_size(lv_radar_cont,lcddev.width,lcddev.height);
    /* 设置背景颜色 */
    lv_obj_set_style_bg_color(lv_radar_cont,lv_palette_main(LV_PALETTE_GREY),LV_STATE_DEFAULT);
    /* 设置改容器不可点击 */
    lv_obj_add_flag(lv_radar_cont, false);
    lv_add_time(lv_radar_cont);   /* 添加选项卡 */
    lv_img_show(lv_radar_cont);   /* 第一个页签 */
}
