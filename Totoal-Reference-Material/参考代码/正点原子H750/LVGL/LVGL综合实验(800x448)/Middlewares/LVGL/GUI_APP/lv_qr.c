...
void lv_lower_case_keyboard(lv_obj_t *parent);
void lv_number_keyboard(lv_obj_t *parent);
void lv_english_keyboard(lv_obj_t *parent);
...
/* 英文键盘 */
static const char *default_kb_map_lc[44] = {"数", "q", "w", "e", "r", "t", "y", "u", "i", "o", "p", LV_SYMBOL_LEFT, "\n",
                                            "ABC", "a", "s", "d", "f", "g", "h", "j", "k", "l","/", "\n",
                                            "_", "-", "z", "x", "c", "v", "b", "n", "m", ".", ",", ":", "\n",
                                            LV_SYMBOL_CLOSE,LV_SYMBOL_PREV," ", LV_SYMBOL_NEXT, LV_SYMBOL_OK, ""};

/* 数字键盘 */
static const char* btnm1_map[21]={"1","2","3","\n",
                                  "4","5","6","\n",
                                  "7","8","9","\n",
                                  "*","0","#","\n",
                                  LV_SYMBOL_OK,LV_SYMBOL_LEFT,LV_SYMBOL_CLOSE,"英",""};
/* 大写英文键盘 */
static const char * default_kb_map_uc[44] = {"!", "Q", "W", "E", "R", "T", "Y", "U", "I", "O", "P", LV_SYMBOL_LEFT, "\n",
                                             "_", "A", "S", "D", "F", "G", "H", "J", "K", "L", LV_SYMBOL_OK, "\n",
                                             "abc", "-", "Z", "X", "C", "V", "B", "N", "M", ".", ",", ":", "\n",
                                             LV_SYMBOL_CLOSE,LV_SYMBOL_PREV, " ", LV_SYMBOL_NEXT, "中", ""};
 ...
 * @return 无
 */
void lv_chinese_keyboard_create(lv_obj_t * par, const lv_obj_t * copy)
{
...
static void lv_event_handler(lv_event_t *event)
{
    ...
    if (code == LV_EVENT_DRAW_PART_BEGIN)
    {
        ...
        else if  (obj == ext->lv_lower_btnm2)
        {
            if (dsc->id > 14)
            {
                dsc->label_dsc->font= &lv_font_montserrat_14;
        ...
        else if  (obj == ext->lv_english_btnm1)
        {
            if (dsc->id < 38)
            {
                dsc->label_dsc->font= &lv_font_montserrat_14;
    ...
    if(code == LV_EVENT_VALUE_CHANGED)
    {
        ...
        if (strstr(txt, "英") != 0)             /* 英 */
        {
            lv_obj_del(obj);                    /* 先删除在创建 */
            obj = NULL;
            lv_english_keyboard(lv_qr_cont);
            return;
        }
        else if (strstr(txt, "abc") != 0)      /* adc */
        {
            lv_obj_del(obj);                    /* 先删除在创建 */
            obj = NULL;
            lv_english_keyboard(lv_qr_cont);
        ...
        else if (strstr(txt, "中") != 0)       /* 中 */
        {
            lv_obj_del(obj);
            obj = NULL;
            lv_chinese_keyboard_create(lv_qr_cont,NULL);
            return;
        }
        else if (strstr(txt, "数") != 0)       /* 数 */
        {
            lv_obj_del(obj);
            obj = NULL;
            lv_number_keyboard(lv_qr_cont);
            return;
        }
        else if (strstr(txt, "ABC") != 0)       /* ADC */
        {
            lv_obj_del(obj);
            obj = NULL;
            lv_lower_case_keyboard(lv_qr_cont);
            return;
        }
        else if(strstr(txt, LV_SYMBOL_OK) != 0||strstr(txt, LV_SYMBOL_CLOSE) != 0) /* 按下OK键或者CLOSE */
        {
            ext->mode = LV_KEYBOARD_MODE_T9;
...
void lv_lower_case_keyboard(lv_obj_t *parent)
{
    ext->lv_lower_btnm2 = lv_btnmatrix_create(parent);
    lv_btnmatrix_set_map(ext->lv_lower_btnm2, default_kb_map_uc);
    lv_btnmatrix_set_btn_width(ext->lv_lower_btnm2, 12, 1);
    lv_obj_set_size(ext->lv_lower_btnm2,lcddev.width,lcddev.height/2);
    lv_obj_align_to(ext->lv_lower_btnm2, NULL, LV_ALIGN_BOTTOM_MID, 0, 0);
    lv_obj_set_style_text_font(ext->lv_lower_btnm2,&myFont14,LV_STATE_DEFAULT);     /* 设置文本字体 */
    lv_obj_add_event_cb(ext->lv_lower_btnm2, lv_event_handler,LV_EVENT_ALL,NULL);
}
...
void lv_number_keyboard(lv_obj_t *parent)
{
    ext->lv_number_btnm1 = lv_btnmatrix_create(parent);
    lv_btnmatrix_set_map(ext->lv_number_btnm1, btnm1_map);
    lv_btnmatrix_set_btn_width(ext->lv_number_btnm1, 12, 1);
    lv_obj_set_size(ext->lv_number_btnm1,lcddev.width,lcddev.height/2);
    lv_obj_align_to(ext->lv_number_btnm1, NULL, LV_ALIGN_BOTTOM_MID, 0, 0);
    lv_obj_set_style_text_font(ext->lv_number_btnm1,&myFont14,LV_STATE_DEFAULT);     /* 设置文本字体 */
    lv_obj_add_event_cb(ext->lv_number_btnm1, lv_event_handler,LV_EVENT_ALL,NULL);
}
...
void lv_english_keyboard(lv_obj_t *parent)
{
    ext->lv_english_btnm1 = lv_btnmatrix_create(parent);
    lv_btnmatrix_set_map(ext->lv_english_btnm1, default_kb_map_lc);
    lv_btnmatrix_set_btn_width(ext->lv_english_btnm1, 13, 1);
    lv_obj_set_size(ext->lv_english_btnm1,lcddev.width,lcddev.height/2);
    lv_obj_align_to(ext->lv_english_btnm1, NULL, LV_ALIGN_BOTTOM_MID, 0, 0);
    lv_obj_set_style_text_font(ext->lv_english_btnm1,&myFont14,LV_STATE_DEFAULT);     /* 设置文本字体 */
    lv_obj_add_event_cb(ext->lv_english_btnm1, lv_event_handler,LV_EVENT_ALL,NULL);
}
...
void lv_ta_add_keyboard_handle(const char * txt,lv_obj_t *parent,lv_obj_t *ta)
{
    ...
    else if (strstr(txt, "1#") != 0)
    {
        ...
        ext->lv_t9_btnm = NULL;               /* T9键盘控制块 */
        ext->roller_py = NULL;                /* 选择拼音控制块 */
        ext->mode = LV_KEYBOARD_MODE_NUM;     /* 数字键盘模式 */
        lv_number_keyboard(lv_scr_act());     /* 创建数字键盘 */
    }
}
...
void lv_ta_action_click(lv_event_t *event)
{
    ...
    if(code == LV_EVENT_CLICKED)    /* 文本区域点击 */
    {
        ...
        else if (ext->lv_number_btnm1 == NULL && ext->mode == LV_KEYBOARD_MODE_NUM)
        {
            lv_number_keyboard(lv_scr_act()); /* 数字键盘 */
            
        }
        else if (ext->lv_english_btnm1 == NULL && ext->mode == LV_KEYBOARD_MODE_CAPITAL_ENGLISH)
        {
            lv_english_keyboard(lv_scr_act()); /* 英文键盘 */
        }
        else if (ext->lv_lower_btnm2 == NULL && ext->mode == LV_KEYBOARD_MODE_LOW_ENGLISH)
        {
            lv_lower_case_keyboard(lv_scr_act()); /* 小写键盘 */
        }
    }
}
