...
char *const FONT_UPDATE_REMIND_TBL[8] =
{
    ...
    "Updating Font24.BIN",          /* 提示正在更新Font26 */
...
uint8_t fonts_update_font(uint16_t x, uint16_t y, uint8_t size, uint8_t *src, uint16_t color)
{
    ...
    for (i = 0; i < 8; i++) /* 先查找文件UNIGBK,GBK12,GBK16,GBK24,GBK32,LVGL12.BIN,LVGL24.BIN,LVGL36BIN是否正常 */
    {
    ...
    if (rval == 0)          /* 字库文件都存在. */
    {
        ...
        for (i = 0; i < 8; i++) /* 依次更新UNIGBK,GBK12,GBK16,GBK24,GBK32,LVGL12.BIN,LVGL24.BIN,LVGL36BIN */
        {
