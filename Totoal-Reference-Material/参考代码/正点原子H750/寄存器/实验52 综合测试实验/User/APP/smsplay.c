...
uint8_t sms_read_sms(sms_node *pnode, uint16_t index, uint8_t mode)
{
    ...
    for (t = 0; t < 3; t++)     /* 尝试3次 */
    {
        ...
        if (res == 0) /* 读取短信成功 */
        {
            ...
            if (strstr((const char *)(USART2_RX_BUF), "OK") == 0) /* 没有收到OK应答,则等待100ms,等OK传送过来 */
            {
                continue;/* 继续下一次 */
            }

            if (strstr((const char *)(USART2_RX_BUF), "UNREAD") == 0)pnode->readed = 1;
            else pnode->readed = 0;
...
uint8_t sms_play(void)
{
    ...
    if (head->item) /* item有数据 */
    {
        ...
        while (1)
        {
            res = sms_msg_select(&selitem, &topitem, APP_MFUNS_CAPTION_TBL[22][gui_phy.language], head);
