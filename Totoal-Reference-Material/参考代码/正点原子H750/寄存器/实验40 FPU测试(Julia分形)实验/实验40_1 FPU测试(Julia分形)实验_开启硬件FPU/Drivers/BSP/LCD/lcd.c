...
void lcd_init(void)
{
    ...
    else if (lcddev.id == 0X9341  || lcddev.id == 0x5310|| lcddev.id == 0x7796)
    {
        ...
        LCD_FMC_BWTRX &= ~(0XF << 0);   /* 地址建立时间(ADDSET)清零 */
        LCD_FMC_BWTRX &= ~(0XF << 8);   /* 数据保存时间清零 */
        LCD_FMC_BWTRX |= 3 << 0;        /* 地址建立时间(ADDSET)为3个fmc_ker_ck=13.5ns */
        LCD_FMC_BWTRX |= 3 << 8;        /* 数据保存时间(DATAST)为3个fmc_ker_ck=13.5ns */
    }
    else if (lcddev.id == 0x9806 || lcddev.id == 0x5510 || lcddev.id == 0X1963)
    {
        /* 重新配置写时序控制寄存器的时序 */
        LCD_FMC_BWTRX &= ~(0XF << 0);   /* 地址建立时间(ADDSET)清零 */
        LCD_FMC_BWTRX &= ~(0XF << 8);   /* 数据保存时间清零 */
