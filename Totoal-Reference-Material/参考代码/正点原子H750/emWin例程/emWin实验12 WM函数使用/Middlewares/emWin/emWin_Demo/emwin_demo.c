...
void emwin_main(void) 
{
    ...
    while (1)
    {
        _DemoSetDesktopColor();     /* 设置窗口的背景颜色 */
        _DemoCreateWindow();        /* 创建子窗口，主要演示WM_CreateWindowAsChild()函数 */
        ...
        _DemoBringToBottom();       /* 将指定窗口放在其他同属窗口的前面，主要展示WMBringToBottom()函数的使用 */
        _DemoMoveWindow();          /* 移动窗口指定的距离，主要展示WM_MoveWindow()函数的使用WM_MoveWindow()函数是把窗口移动指定距离 */
